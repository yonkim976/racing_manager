...
- 모든 레이스는 Physics V2에서 실행되며 런타임 엔진 선택 기능은 제공하지 않는다.
- Physics V2는 트랙 곡률의 실제 제한속도를 사용하고 50Hz 고정 서브스텝으로 속도·가속도·거리를 적분한다.
- 서킷 `base_lap_time`은 매 틱 차량을 이동시키는 값이 아니라 물리 속도 프로파일을 한 번 보정하는 기준으로 사용한다.
- 랩타임은 물리적으로 스타트/피니시 라인을 통과한 결과로 기록된다.
- 앞차의 시작·종료 궤적을 이용해 50Hz마다 추종 속도를 보정하며, 1.9m 차폭이 좌우로 겹칠 때는 최소 5.0m의 종방향 차체 간격을 유지한다.
- 중심선을 기준으로 코너 곡률에서 레이싱 라인을 자동 생성한다. 그린 플래그의 승인된 attack/side-by-side 상태는 실제 미터 단위 좌우 목표로 이동하며, 차체 중심이 최소 2.15m 분리되어야 추월할 수 있다.
- Canvas 확대는 `100/250/500/1500/2000%`의 5단계이며, `1500%`와 `2000%`에서는 기본 12m 트랙 폭, 촘촘하게 보간한 자동 레이싱 라인과 차량을 동일한 실제 비율로 표시하고 플레이어 차량을 자동 추적한다. 추적 중에는 정적 트랙을 다시 그리지 않고 Pixi 월드 위치만 이동한다.
- VSC와 일반 SC에서는 트랙 추월이 차단되고, 명시적인 언랩 차량만 예외다.
- 자세한 경계와 다음 단계는 `PHYSICS_V2_DESIGN.md`를 참고한다.
...
- 차량은 순간이동하지 않고 거리별 추격 속도로 10대 차량 길이(50m) 안에서 합류하며, 이후 7대 차량 길이(35m) 목표 간격으로 점진적으로 대열을 압축한다
