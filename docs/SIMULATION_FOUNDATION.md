...
- 전·후축 실제 구동·제동 작업량과 슬라이드 에너지, 2노드 표면·코어 열수지 및 수치 guard 진단
- 차체 충돌 외곽선, 접촉점과 충격량
...
- 화면 pose는 실시간 `30Hz` 바이너리로 보내며 직전 전송 이후의 50Hz 권위 pose 열만 포함한다. 순위·간격·전략 상태는 `4Hz`, 27개 미니섹터 비교는 `1Hz`, 완주 랩 기록은 최초 전체 snapshot 뒤 드라이버별 증분, 공개 이벤트는 발생 시 별도 메시지로 전송한다. 일시정지 중 pose와 대시보드는 `1Hz`로 낮춘다. 현재 활성화된 Three.js 캔버스는 공용 pose 재생 모듈로 pose를 최대 `60FPS`로 보간한다. PixiJS 렌더러는 롤백용 소스만 유지한다.
- 프런트 pose 재생은 차량별 160칸 typed-array 링 버퍼를 사용하고 보간 출력 객체를 재사용한다. 2.5초보다 오래된 표본은 논리 head만 이동해 제거하며 배열 `shift()`와 표본별 JS 객체 누적을 허용하지 않는다.
- 50Hz 프레임 ID와 step 간격은 전체 레이스 이력이 아니라 최근 256개 진단 표본만 유지한다. 원본 타이밍 루프 통과 시각은 완료 랩 요약을 만든 뒤 최근 4개 랩 범위만 남기며, 장기 기록은 sector·mini-sector가 포함된 `LapTimeInfo`가 담당한다.
- 세션 루프는 벽시계 기반 시뮬레이션 credit과 화면 전송 deadline을 분리한다.
- 지원 배속은 `1x/2x`이며 같은 시뮬레이션 시간에는 같은 물리·AI 결과가 나와야 한다.
...
- 랩은 실제 F1 표시와 같이 3개 공식 대섹터(S1/S2/S3)로 유지한다.
- GAP은 선두와의 차이, INT는 바로 앞 차량과의 차이를 뜻하며 화면상 직선거리나 두 차량의 유클리드 거리로 계산하지 않는다.
- 바레인은 대섹터 안을 S1 9개, S2 12개, S3 6개의 총 27개 고정 타이밍 루프로 나눈다. 평균 간격은 약 200m이며 각 차량이 같은 루프를 통과한 보간 시각끼리 비교한다.
- 공식 타이밍 루프 통과 시각을 기준점(anchor)으로 유지하면서, 루프 사이에서는 현재 트랙 진행 거리와 속도를 혼합해 값을 연속 갱신한다. 유효한 공통 기준점이 있으면 `LIVE`, 아직 없거나 추월 직후 최신 통과 순서가 뒤집힌 경우에는 `~`와 `EST`로 표시한다. 뒤집힌 과거 기록을 `0.000`으로 강제하지 않는다.
- 미니섹터 기록은 세션 최고(보라), 개인 최고(초록), 그 외 유효 기록(노랑)으로 비교한다. 그래픽의 대·미니섹터 선은 이 물리 타이밍 루프와 같은 진행 좌표를 사용한다.

## 3. 현재 물리 모델의 위치

현재 구현은 트랙 진행 좌표의 종방향 힘 모델과 평면 dynamic bicycle model을 결합한다. 전·후축 슬립각과 횡력, 3.4m 휠베이스, 요 관성, 조향각, 차체 헤딩과 요레이트를 50Hz에서 적분하며 공력, 구동력, 제동력, 그립 한계와 차체 외곽선 충돌도 같은 권위 상태를 사용한다. 실제 종가속도에 따른 준정적 전·후축 하중 이동, 축별 평균 회전속도·슬립률, 발생 지점에서 분리한 전·후축 슬라이드/작업 에너지, 전·후축 타이어 표면/코어 열과 브레이크 열·냉각·fade도 계산한다. 타이어 열 모델의 대기·노면 경계조건은 세션 소유 `TrackConditions`에서 매 스텝 명시적으로 전달하며, 표면·코어 수치 guard의 적용 전 overshoot는 bounded 진단으로 보존한다. 다만 네 바퀴 각각의 회전 관성·수직하중·열·마모와 서스펜션까지 적분하는 완전한 차량 동역학 모델은 아니다.

현실성 향상 순서는 다음과 같다.

1. **현재 기반 안정화**: 실제 이동 상태에서만 랩, 추월, 충돌, 이벤트를 확정한다.
2. **완료 — 2D bicycle model**: 전·후축 횡력, 요 관성, 조향각, 차체 헤딩과 요레이트를 상태로 적분한다.
3. **부분 완료 — 결합 타이어·축 하중 한계**: 제동·가속·코너링이 동일한 마찰 원을 사용하며 락업·휠스핀과 힘 손실을 계산한다. 종가속도로 전·후축 하중을 나누고 축별 평균 슬립을 산출한다. 다음 정밀화 대상은 타이어 압력과 네 바퀴 독립 상태다.
4. **부분 완료 — 열·질량 상태**: 전·후축 타이어 표면/코어 온도, 전·후축 브레이크 온도·fade와 연료 소모 질량이 실제 힘 한계에 반영됐다. 기존 차량 전체 타이어 온도는 두 축 평균으로 호환 유지한다. 네 바퀴 독립 열·마모와 브레이크 냉각 설계 변수는 후속 단계다.
...
- Bahrain은 공식 폭 범위 `14–22m`와 T1 `22m` 제어점을 적용했다. 좌우 측량값은 공개되지 않았으므로 T1 안쪽은 기준 반폭 7m를 유지하고 확장분을 바깥쪽에 배분한 복원값을 사용한다. Red Bull Ring은 TUM의 가변 폭 샘플을 적용했고 Silverstone·Spa·Hungaroring은 실제 좌우 폭 샘플이 없어 전 구간 기본 폭 12m를 사용한다.
...
트랙 리밋은 네 바퀴 접촉점이 모두 흰 선 바깥 표면일 때만 활성화한다. 낮은 연석은 기본 최적선이 전폭을 목표로 삼지 않도록 0.60m만 최적화 폭에 포함하지만, 교통·추월 후보의 차체 경계 검사는 낮은 연석 전체 폭을 사용할 수 있다. 따라서 두 바퀴를 낮은 연석에 올리고 나머지 두 바퀴를 트랙에 둔 코너 통과는 유효하며, 높은 연석과 런오프는 같은 허용 폭으로 취급하지 않는다.

### 6.1 코너 공략과 주행 규칙

FIA Appendix L의 기본 원칙은 직선 방어 중 방향 변경을 한 번만 허용하고, 레이싱라인으로 복귀할 때 트랙 가장자리와 차 한 대 폭을 남기며, 제동이 시작된 뒤에는 정상적인 레이싱라인 추종 외의 방향 변경을 금지한다. 코너 점유는 “무조건 안쪽 우선”이 아니다. 안쪽 공격자는 늦어도 apex에서 앞축이 상대 mirror까지 와야 하고 제어된 합리적 라인을 유지해야 한다. 바깥 차량도 apex에서 앞축이 상대 앞축보다 앞서고 진입·apex·탈출 내내 제어된 상태라면 공간을 받을 수 있다.

구현은 이 규칙을 다음처럼 해석한다.

1. 추월 후보는 안쪽과 바깥쪽을 동일한 물리 horizon으로 평가한다.
2. 코너 제동 preview에 들어가기 전에 공격·방어 corridor와 레이싱라인 상대 횡위치를 고정한다.
3. 고정 뒤에는 트랙 곡률을 따라가되 상대 차량에 반응한 두 번째 방향 변경은 허용하지 않는다.
4. apex의 front-axle 위치, 차체 clearance, 제동 여유와 경계를 모두 만족해야 side-by-side 코너 점유를 승인한다.
5. 승인되지 않은 차량은 횡으로 튕겨나가지 않고 현재 corridor에서 종방향으로 양보한 뒤 복귀한다.

FIA 기준에서 흰 선은 트랙의 일부지만 연석은 트랙이 아니다. 따라서 연석 접촉 자체를 불법으로 보지 않고, 최소 한 바퀴의 접촉점이 흰 선 안쪽에 남는지와 연석 높이에 따른 실제 그립 손실을 별도로 계산한다. 낮은 연석은 lateral/braking/traction grip이 각각 `0.96/0.94/0.90`, drag 감속은 `0.15m/s²`로 작게 불리하다. 빠른 출구를 만드는 이득이 이 비용보다 크면 최적화가 낮은 연석 일부를 사용할 수 있지만 높은 연석은 같은 방식으로 허용하지 않는다.
...
코너 진입 점유가 거절된 차량은 목표 횡위치를 즉시 레이싱라인으로 바꾸지 않는다. 현재 corridor를 유지한 `yield` 단계에서 차체 전체와 최소 범퍼 간격이 뒤로 빠질 때까지 종방향으로 양보하고, 그 뒤에만 저장된 실제 횡위치에서 레이싱라인까지 smoothstep으로 복귀한다. 이 전환은 물리 pose를 직접 이동시키지 않으며 50Hz 저수준 제어기가 연속적으로 추종한다.
...
3. **완료**: 1x/2x soak, 피트·다중 추월·후류·실데이터 게이트와 20대 30분 플래너 검증
...
10. **부분 완료**: 준정적 전·후축 하중 이동, 축별 회전속도·슬립, 전후축 타이어 열, 브레이크 열·냉각·fade와 데이터 센터 상태 표시
11. **1차 완료**: 바레인 AI 전술 선택 벤치마크. 다음은 실제 리플레이 분포 보정 후 필요 시 강화학습·모방학습 실험
12. 2026 파워유닛, 에너지 관리와 액티브 에어로는 별도 ruleset으로 구현
