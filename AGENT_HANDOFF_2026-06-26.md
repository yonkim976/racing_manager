...
- 미합류 차량은 앞 대열과의 거리에 따라 `×1.08~×1.25`, SC 대열 차량은 기본 `×1.8`로 주행합니다. 현재 물리 차체 길이 5.0m를 기준으로 10대 차량 길이(50m) 안에서 합류로 판정하고, 합류 후에는 7대 차량 길이(35m) 목표 간격까지 완만하게 압축합니다. 다시 50m 밖으로 벌어지면 자동으로 추격 상태로 복귀합니다.
