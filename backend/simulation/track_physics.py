...
def clear_vehicle_track_physics_cache() -> None:
    ...
    from engines.full.runtime.vehicle_track_solver import (
        clear_vehicle_track_physics_cache as clear_full_vehicle_cache,
    )

    clear_full_vehicle_cache()


def track_physics_cache_counts() -> dict[str, int]:
    """Expose cache sizes without serializing any track profile data."""
    from engines.full.runtime.vehicle_track_solver import (
        vehicle_track_physics_cache_count,
    )

    return {
        "common_track_physics_cache_entries": len(_TRACK_PHYSICS_CACHE),
        "vehicle_track_physics_cache_entries": vehicle_track_physics_cache_count(),
    }


_FULL_VEHICLE_SOLVER_EXPORTS = frozenset(
    {
...
def __getattr__(name: str):
    """Lazily preserve legacy vehicle-solver imports without loading FULL."""
    if name in _FULL_VEHICLE_SOLVER_EXPORTS:
        from engines.full.runtime import vehicle_track_solver

        return getattr(vehicle_track_solver, name)
